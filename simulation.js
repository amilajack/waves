const Camera = function () {
  ...
  this.changeAzimuth = (deltaAzimuth) => {
    azimuth += deltaAzimuth;
    azimuth = clamp(azimuth, MIN_AZIMUTH, MAX_AZIMUTH);
    changed = true;
  };

  this.changeElevation = (deltaElevation) => {
...
const Simulator = function (canvas, width, height) {
  var canvas = canvas;
  canvas.width = width;
  ...
  let windY = INITIAL_WIND[1];
  let size = INITIAL_SIZE;
  let choppiness = INITIAL_CHOPPINESS;
  ...
  this.setChoppiness = (newChoppiness) => {
  ...
  this.render = (deltaTime, projectionMatrix, viewMatrix, cameraPosition) => {
    ...
    for (let i = 0; i < iterations; i += 1) {
      ...
      gl.uniform1f(subtransformProgram.uniformLocations.u_subtransformSize, 2 ** (i % (iterations / 2) + 1));
      gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4);
    }
